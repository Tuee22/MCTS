...
thread_local std::vector<State> tls_move_buffer;

[[gnu::hot]] static uint8_t decode_action_id(const State &state) noexcept {
...
[[gnu::hot]] static void legal_moves_for_state(const State &state, std::vector<State> &moves) {
    moves.clear();
    state.append_legal_successors(moves);
}
...
[[gnu::hot]] static void expand(Arena &arena, uint32_t node_idx, uint16_t max_plies) {
    ...
    for (uint32_t i = 0; i < n; ++i) {
        ...
        child.state = std::move(tls_move_buffer[i]);
...
[[gnu::hot]] static double rollout(const State &start, uint64_t seed, uint16_t max_plies) {
    ...
    for (uint16_t step = 0; step < max_plies; ++step) {
        ...
        current = std::move(tls_move_buffer[pick]);
    ...
    double exploration_c)
{
    ...
    for (uint16_t i = 0; i < root.n_children; ++i) {
        ...
        out.visits.emplace_back(decode_action_id(child.state), child.visit_count);
    ...
    out.chosen_action_id = decode_action_id(chosen.state);
    out.chosen_equity =
