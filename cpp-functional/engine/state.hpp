...
//
// The search boundary treats State as an immutable value: legal-successor
// generation appends child States, and C ABI move application uses
// `try_advance` to select a validated transition. The implementation keeps the
// same compact wall bitfields and direct numeric action order as the C++
// steelman/Rust backends so representation cost is not part of the
// functional-core comparison.
...
struct State {
    ...
    [[gnu::always_inline]] inline uint8_t action_id() const noexcept {
        return last_action;
    }

    template <typename Output>
    [[gnu::hot]] void append_legal_successors(Output &output) const {
...
private:
    ...
    [[gnu::always_inline]] inline State flipped_after_move() const noexcept {
        State out;
        ...
        out.ply_count = static_cast<uint16_t>(ply_count + 1);
        out.last_action = last_action == kNoAction ? kNoAction : flip_action_id(last_action);
        return out;
    }

    [[gnu::always_inline]] inline State child_after_action(uint8_t action_id) const noexcept {
        State child = *this;
        child.apply_action_unchecked(action_id);
        return child.flipped_after_move();
    }
...
[[gnu::hot]] inline std::optional<State> try_advance(const State &current, uint8_t action_id) {
    std::array<State, 16> moves{};
    size_t count = 0;
    struct SmallStateSink {
        std::array<State, 16> &items;
        size_t &size;

        [[gnu::always_inline]] inline void emplace_back(State &&state) noexcept {
            if (size < items.size()) {
                items[size++] = state;
            }
        }
    ...
    current.append_legal_successors(sink);
    for (size_t i = 0; i < count; ++i) {
        if (moves[i].action_id() == action_id) {
            return std::optional<State>{moves[i]};
        }
    }
    return std::nullopt;
}
