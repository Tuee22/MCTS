// Compact functional-core Corridors state for backend (iii).
//
// Sprint 6.9 adopts the backend-(ii) hot-path shape inside the
// functional-core boundary: an absolute `SideToMove` field replaces the
// per-transition full-state coordinate/wall flip, a reusable `BlockMasks`
// value is precomputed once per `legal_actions` call, wall-candidate
// legality runs against an additively-extended mask, and
// `path_exists_with_masks` is a bidirectional bit-parallel BFS. The
// functional-style API surface (value semantics at the boundary,
// `try_advance` returning `std::optional<State>`) is preserved; the
// `State` value is the canonical immutable snapshot the search materializes
// on the descent stack rather than embedding inside every arena node.
...
// Fixed-capacity stack buffer for the at-most-16 legal action IDs
// emitted by `legal_actions`. No heap allocation in the hot path.
struct ActionBuffer {
...
// Precomputed wall-block masks. Mirrors backend (ii)
// `cpp-imperative/engine/fast_board.hpp::BlockMasks`. Computed once per
// `legal_actions` call via `block_masks()` and additively extended per
// wall candidate via `add_wall_to_masks()`; the per-candidate State copy
// and inline mask recomputation that lived on the hot path before Sprint
// 6.9 are removed.
struct BlockMasks {
...
struct State {
    ...
    [[gnu::hot, gnu::always_inline]] inline bool hero_wins() const noexcept {
        return hero_y == kBoardSize - 1;
    }

    [[gnu::hot, gnu::always_inline]] inline bool villain_wins() const noexcept {
        return villain_y == 0;
    }
    ...
    // Hero-perspective terminal evaluation; ply-cap termination is a draw.
    ...
    // Canonical action-ID flip used at the C ABI boundary and for
    // canonical legal-action ordering. Identical to the imperative
    // helper.
    ...
    // hero-perspective convention the C ABI shim reports through
    // `mcts_functional_search_move` / `recompute_move`.
    ...
    // Inverse of `abi_action_from_absolute` used when the C ABI hands
    // an operator move into the engine.
    ...
    [[gnu::always_inline]] inline uint8_t action_id() const noexcept { return last_action; }

    // Direct capped legal-action generation in absolute action IDs.
    [[gnu::hot]] void legal_actions(ActionBuffer &output) const noexcept {
        ...
        for (uint16_t canonical = 81; canonical <= 208 && wall_count < 12; ++canonical) {
            const uint8_t aid = static_cast<uint8_t>(canonical);
            if (wall_action_exists(aid) || !wall_action_legal(aid, base_masks)) {
                continue;
            }
            output.push(aid);
            ++wall_count;
    ...
    // Trusted internal transition; toggles `side_to_move` and increments
    // `ply_count`. Callers must already know the action is legal.
...
private:
    ...
    [[gnu::always_inline]] inline bool wall_action_legal(
        ...
        return path_exists_with_masks(true, trial_masks)
            && path_exists_with_masks(false, trial_masks);
    }
    ...
    // Sprint 6.9: bidirectional bit-parallel BFS. Two simultaneous
    // frontiers (start cell and goal row) expand under the same
    // four-direction shift+mask kernel; the function returns true the
    // moment the two visited sets intersect. Walls are bidirectional in
    // Corridors, so the shared `BlockMasks` is valid for both
    // expansions. The bool return contract is unchanged versus the
    // prior unidirectional implementation; Q3 visit payloads remain
    // bit-identical.
...
// Value-style transition: accepts a legacy-ABI hero-perspective action
// ID, validates it against the current `legal_actions` set, and returns
// the successor `State`. Returns `std::nullopt` when the ABI action is
// not legal from `current`. The functional-core boundary is preserved:
// callers see an immutable value transition.
[[gnu::hot]] inline std::optional<State> try_advance(const State &current, uint8_t abi_action_id) noexcept {
    const uint8_t absolute_action_id = current.absolute_action_from_abi(abi_action_id);
    ActionBuffer actions;
    current.legal_actions(actions);
    for (size_t i = 0; i < actions.size; ++i) {
        if (actions[i] == absolute_action_id) {
            State next = current;
            next.apply_action_unchecked(absolute_action_id);
            return std::optional<State>{next};
        }
    }
    return std::nullopt;
}
