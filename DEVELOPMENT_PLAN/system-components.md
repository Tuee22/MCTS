...
| Rollouts verify | `mcts verify rollouts` | Logical round-robin visit-count equality across the `(ii)..(v)` cohort under `--rng cpp`; logical layered envelope checks with `--allow-stale` | 🔄 Active (`MCTS.Verify` uses `Driver.runBatch` so Q3 is independent of live foreign cdylib drift; focused `mcts-cross-backend` rollout smoke passes and fails on `VerifyMismatch`) | Sprint 7.2, Sprint 7.5 |
| Self-play verify | `mcts verify selfplay` | Logical round-robin self-play visit-count equality across the `(ii)..(v)` cohort | 🔄 Active (`MCTS.Verify` uses `Driver.runBatch` so Q3 is independent of live foreign cdylib drift; focused `mcts-cross-backend` self-play smoke passes and fails on `VerifyMismatch`) | Sprint 7.2, Sprint 7.5 |
| Legacy parity verify | `mcts verify legacy-parity {rollouts\|selfplay}` | Logical 5-backend round-robin under `max_plies = 10000`, pinned fixture seed, `--rng cpp`; workload dispatched by `LegacyParityWorkload` | 🔄 Active (Q6 fixtures are refreshed at `S_LP_SIMS = 10000`; live backend (i) pre-flight remains in `mcts-integration`) | Sprint 7.2 |
| Interactive game | `mcts play` | TUI human-vs-AI or AI-vs-AI spectate with live board rendering | 🔄 Active (non-interactive fallback plus interactive `brick` `App` event loop in `MCTS.CLI.Tui.Play` with legacy-notation move input (`MCTS.Notation.parseMove`), `:hint`/`:undo`/`:quit`, `:save` transcript writes through `MCTS.Transcript.writePlayTranscript`, selected-backend AI advance through `MCTS.Driver.ForeignSearch.foreignSearchMove` when a foreign shared library is present, logical fallback when it is absent, and a pure `applyUserInput` dispatcher plus write/decode and AI-advance assertions covered by `mcts-unit::exerciseTuiPlayInput`; the shared board widget renders pawn cells plus horizontal/vertical wall segments from pure text rows pinned by `test/golden/cli/tui-board.txt`) | Sprint 7.4 |
...
| Divergence matrix | `mcts inspect divergence <hash-prefix>` | Emit divergence-rate metrics (`visit-Δ`, `move-Δ`, `equity-L2`) for a single transcript and cached backend columns | 🔄 Active (`divergenceVsEqStream` pairs the transcript against each cached sidecar `EqStream`; per-sidecar metrics render with real `equity_l2_drift`; live foreign-recompute rows are added for available cpp-imperative, cpp-functional, and Rust cdylibs; `MCTS.ReportCard.reportDivergenceRows` renders the report-card table/JSON matrix, and `MCTS.ReportCard.divergenceRowsFromTranscripts` populates `mcts test all` rows from the measured logical `G_V` verify cohort) | Sprint 7.5 |
| Test runner | `mcts test all` / `mcts test <stanza>` | Plan/Apply over backend builds, Cabal test stanzas, verify cohorts, and the no-write measured report-card workload | 🔄 Active (`--dry-run`; recursive CLI steps route through `cabal exec mcts -- ...`; report-card summary plus Q1/Q2/Q5 measured fields and divergence matrix table/JSON goldens exist; final bounded Q1-Q7 evidence remains open) | Sprint 7.3 |
...
|--------|------|-------|--------|---------------|
| `mcts-unit` | Pure logic | Current `test/unit/Main.hs` uses a fine-grained `tasty` / `tasty-hunit` runner with 26 named cases grouped by CLI/parser, transcripts/cache, engine/RNG, envelopes/sidecars, plans/subprocesses, and renderers/TUI dispatch. Coverage includes command registry/parser smoke, `execParserPure` happy/failure paths, SHA-256, all-action notation roundtrips, transcript roundtrip, four backend-tagged transcript byte goldens, known-position engine golden, envelope/cache-key checks, sidecars, `inspect show`, `inspect list --format json`, `renderError`, subprocess/render goldens, TUI board/status layout, divergence metrics, and split engine/RNG/FFI fixture checks; target promotes fixture-scale properties and byte-golden assertions into dedicated `tasty-quickcheck` / `tasty-golden` providers where useful | 🔄 Active (named tasty split present; property/golden provider promotion open) | Sprint 7.1 |
| `mcts-integration` | Logical integration baseline plus bounded FFI smoke | Current `test/integration/Main.hs` uses a `tasty` runner and checks same-backend determinism directly through `MCTS.Driver`, real `mcts` binary same-backend determinism through `MCTS.Subprocess.capture` for Haskell and every built foreign backend, sidecar origin markers, bounded measured report-card divergence building, cached recompute-sidecar consumption through the real `mcts inspect divergence` subprocess, bounded dynamic-FFI smoke games, live `mcts_<backend>_get_envelope` loading, live transcript-envelope stamping, backend-slot stale hard-fail/`--allow-stale` warning behavior for `cpp-legacy`, `cpp-imperative`, `cpp-functional`, and `rust` when their shared libraries are present, and Q6 fixture byte guards over every committed `test/golden/legacy/transcripts/<arch>/*.tr` file | 🔄 Active (real-binary determinism smoke plus bounded report-card/sidecar/FFI smoke/envelope/stale and Q6 fixture coverage; full FFI transcript scale-out remains open) | Sprint 7.1 |
| `mcts-cross-backend` | Round-robin verify | Current `test/cross-backend/Main.hs` uses a `tasty` runner for rollout and self-play logical `verify` cohorts under `--rng cpp` covering backends `(ii)`, `(iii)`, `(iv)`, `(v)` through `Driver.runBatch`; backend `(i)` is rejected by the typed `VerifyBackend` parser/runtime cohort checks, and any `VerifyMismatch` fails the focused cases and the bounded canonical report-card cohort | ✅ Done for logical Q3 evidence; live FFI promotion remains ledger-tracked | Sprint 7.2 |
| `mcts-legacy-parity` | Round-robin verify, legacy envelope | Current `test/legacy-parity/Main.hs` uses a `tasty` runner for logical `verify legacy-parity` across all five backend slots; the typed `LegacyParityBackend` parser path rejects cohorts missing `LpCppLegacy`; target pins `max_plies = 10000`, fixture seed `S_LP = 42`, and Q7 logical round-robin coverage. Q6 external fixture guards and live backend (i) pre-flight live in `mcts-integration` | ✅ Done for logical Q7 evidence; live FFI promotion remains ledger-tracked | Sprint 7.2 |
