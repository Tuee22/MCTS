...
| Build outputs | `docker/Dockerfile`; runtime entry through `docker compose run --rm mcts mcts <command>` | Image-local `dist-newstyle/` plus per-backend `build/` / `target/` directories | Canonical Cabal component artefacts and backend artefacts live inside the Compose-built image; runtime commands consume them without rebuilding, and host-level `.build/` is unsupported. Steelman foreign artefacts are valid only after Dockerfile-time PGO/BOLT succeeds and the installed bolted library smoke passes. |
