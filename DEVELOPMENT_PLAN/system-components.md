...
| In-process MCTS engine | `src/MCTS/Engine.hs`, `src/MCTS/Engine/Envelope.hs`, `src/MCTS/Driver.hs`, `src/MCTS/Search/Arena.hs`, `src/MCTS/Search/UCT.hs`, `src/MCTS/Verify.hs`; the Haskell engine uses compact `Word8` pawn slots plus strict `Word64` wall bitsets, real recursive UCT in the `ST` monad over a structure-of-arrays `STUArray` arena, direct non-terminal action sets, no-ply rollout apply with local ply tracking, single-constructor action transitions with optional ply increment, direct packed-slot path starts in the wall-legality BFS, a no-wall legal-action fast path, signed-modulo rollout move selection per the byte-consumption contract, fused arena visit/value updates, first-unvisited UCT child selection, and per-move search dispatch from the driver. The current driver allocates a fresh arena for each move search; `treeReroot` is a tested primitive, not a game-loop persistence path. `MCTS.Verify` dispatches through `MCTS.Driver.Dispatch.runBatchDispatch`; Q3 reaches `(ii)..(v)`. It compares a canonical byte-projected determinism payload digest first, then runs a length-aware game/move/terminator scan only on digest mismatch. | ✅ Done (Sprint `8.15` closed 2026-05-28 with the measurement-vs-invariant reframe and post-`5.7` `(ii)` rebaseline) |
| Foreign backend homes | `cpp-legacy/` contains the mechanically imported legacy core plus C ABI wrappers and the optional local `legacy-to-wire` evidence generator reached through `mcts build legacy-fixtures`; `cpp-imperative/` contains the imperative steelman with `FastBoard` compact bitfield state, fixed-capacity action-id generation, absolute side-to-move state, action-only tree nodes, reusable wall-block masks, trusted internal apply/cache paths, representative PGO+BOLT training, and wavefront escapability checks; `cpp-functional/` contains the compact functional-core C++ backend with value-state fields in `state.hpp`, direct capped numeric successor generation, bitfield path checks, and search/C ABI transitions that no longer depend on backend-local legacy `board.*` or action text; `rust/src/` ships a real arena MCTS, the splitmix-compatible live search schedule, the full Corridors gameplay port with bit-parallel `u128` path checks, fixed-capacity action buffers, child-bound search arena reservation, and board-handle-local read-visits cache, plus the search/recompute/read-visits/envelope C ABI. Haskell dispatch loads all four foreign backend families through `src/MCTS/FFI/Cpp*`, `src/MCTS/FFI/Rust.hs`, and `src/MCTS/Driver/Dispatch.hs` when canonical shared libraries are present. Sprint `5.5` closed backend (ii)'s compact ABI contract; Sprint `5.6` closed backend (ii)'s compact-board hot path; Sprint `5.7` closed the full backend `(ii)` hot-path steelman; Sprint `6.6` closed backend (iii)/(iv) ABI docs and Rust build-artifact wording; Sprint `6.7` closed backend (iii)'s compact functional-core source-style alignment; Sprint `6.8` closed Rust's hot-path structural alignment. | ✅ Done |
...
| (iv) Rust | `rust` | `rust/`, `src/MCTS/Driver/Rust.hs` | C ABI via Haskell FFI, `cdylib` | ✅ Done for real arena MCTS with UCT-1, splitmix-compatible live search schedule, full search/recompute/read-visits/envelope C ABI, real Corridors gameplay in `rust/src/board.rs`, local `SystemMiMalloc` global allocator over the container `libmimalloc`, single optimized FFI artefact, fail-closed Rust PGO/BOLT plan, Sprint `8.10` bounded played-game profile training, Sprint `8.11` bounded metric-suite profile training, and Sprint `6.8` bit-parallel path checks, fixed-capacity action buffers, child-bound arena reservation, reduced clone churn, and board-handle-local visit caching | [Phase 6](phase-6-cpp-functional-and-rust.md), [Phase 8](phase-8-haskell-performance-parity-closure.md) |
| (v) Haskell | `haskell` | `src/MCTS/Engine.hs`, `src/MCTS/Engine/Envelope.hs`, `src/MCTS/Driver.hs`, `src/MCTS/Search/Arena.hs`, `src/MCTS/Search/UCT.hs` | Native (in-process) | ✅ Done. Phase 3 correctness baseline, compact functional-core transition boundary, Sprint `8.12` parity refresh against corrected backend (ii), Sprint `8.13` style alignment, Sprint `8.14` report-card verdict/sample-stability gate, and Sprint `8.15` measurement-vs-invariant reframe plus post-`5.7` `(ii)` rebaseline all closed. The hot search path uses packed numeric action IDs, compact pawn slots, direct packed-slot path starts, no-wall legal-action fast path, no-ply rollout apply, single-constructor action transitions, and `ST`-local arena internals behind the pure API. | [Phase 3](phase-3-haskell-engine.md), [Phase 8](phase-8-haskell-performance-parity-closure.md) |
