...
reclosure sprints have closed without changing the five-backend hypothesis.
...
| (ii) C++ imperative steelman | `cpp-imperative` | `cpp-imperative/` | C ABI via Haskell FFI | ✅ Done (live parser/build/verify/FFI dispatch, Q3 participation, canonical C++ PGO/BOLT Plan/Apply wiring, and governed compact C ABI contract are closed) | [Phase 5](phase-5-cpp-imperative-steelman.md), [Phase 8](phase-8-haskell-performance-parity-closure.md) |
| (iii) C++ functional-style | `cpp-functional` | `cpp-functional/` | C ABI via Haskell FFI | ✅ Done (live parser/build/verify/FFI dispatch, Q3 participation, shared C++ PGO/BOLT Plan/Apply wiring, and compact ABI wording are aligned) | [Phase 6](phase-6-cpp-functional-and-rust.md), [Phase 8](phase-8-haskell-performance-parity-closure.md) |
| (iv) Rust | `rust` | `rust/`, `src/MCTS/Driver/Rust.hs` | C ABI via Haskell FFI, `cdylib` | ✅ Done (real arena MCTS with UCT-1, splitmix-compatible live search schedule, dormant xoshiro256++ helper module, full search/recompute/read-visits/envelope C ABI, real Corridors gameplay in `rust/src/board.rs`, `mimalloc::MiMalloc` global allocator, single optimized FFI artefact, and Rust PGO/BOLT plan are closed) | [Phase 6](phase-6-cpp-functional-and-rust.md) |
...
| Test runner | `mcts test all` / `mcts test <stanza>` | Plan/Apply over lint/docs prerequisites, warning-clean build, backend builds, Cabal test stanzas, verify cohorts, and the no-write measured report-card workload; exact ordering is owned by `unit_testing_policy.md` | ✅ Done | Sprint 7.2, Sprint 7.3, Sprint 8.8, Phase 8 restoration |
