...
across `(i)..(v)`. Phase `8` closes the restoration while preserving the Haskell
performance-parity proof and the rule that normal validation does not depend on
...
| (ii) C++ imperative steelman | `cpp-imperative` | `cpp-imperative/` | C ABI via Haskell FFI | ✅ Done (live parser/build/verify/FFI dispatch; Q1/Q2 live performance ceiling; Q3 participant) | [Phase 5](phase-5-cpp-imperative-steelman.md), [Phase 8](phase-8-haskell-performance-parity-closure.md) |
| (iii) C++ functional-style | `cpp-functional` | `cpp-functional/` | C ABI via Haskell FFI | ✅ Done (live parser/build/verify/FFI dispatch; style-isolation comparison; Q3 participant) | [Phase 6](phase-6-cpp-functional-and-rust.md), [Phase 8](phase-8-haskell-performance-parity-closure.md) |
...
| Per-backend build harness | `mcts build cpp-legacy`, `mcts build cpp-imperative`, `mcts build cpp-functional`, `mcts build rust`, plus `mcts build legacy-fixtures` | Plan/Apply per-backend build and PGO+BOLT+`mimalloc` pipelines where applicable; optional local legacy evidence generator for Q6 | ✅ Done | Sprint 4.5, Sprint 5.3, Sprint 6.2, Sprint 6.4, Sprint 8.8, Phase 8 restoration |
...
| PGO profile directories | Two-stage PGO build harness | `cpp-imperative/pgo-profile/`, `cpp-functional/pgo-profile/`, `rust/pgo-profile/` | Populated by instrumented builds and consumed by optimised builds |
