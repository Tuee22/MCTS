...
The plan and governed docs remain on the existing doctrine topology; every
implementation surface across Phases `0`–`8` is closed on its owned code.
...
that sprint has now closed the action-id generation, side-to-move, tree-layout,
wall-mask, trusted-buffer, and profile-retuning work. The Sprint `8.14`
report-card verdict line is historical evidence for the Sprint `5.6` backend
`(ii)` artefact. Phase `8` Sprint `8.15` closed on 2026-05-28 with the
...
and the rebaselined Haskell measurement against the fully steelmanned `(ii)`
target. The aggregate `mcts test all` exits 0 with all four apples-to-apples
invariants Q3/Q4/Q6/Q7 PASS, all six Cabal stanzas PASS, zero live-cohort
divergence, and the labelled measurement `Verdict: Trails parity band by 52.3%`
(measurement recorded honestly per the PGO-asymmetry attribution; not a closure
gate). The pre-reframe `Shortfall 0.2678864950323545` rerun against the same
post-`5.7` target remains historical evidence.
...
| (ii) C++ imperative steelman | `cpp-imperative` | `cpp-imperative/` | C ABI via Haskell FFI | ✅ Done for live parser/build/verify/FFI dispatch, Q3 participation, canonical fail-closed C++ PGO/BOLT Plan/Apply wiring, governed compact C ABI contract, Sprint `5.6` compact-board hot path, Sprint `8.10` bounded played-game profile training, Sprint `8.11` bounded metric-suite profile training, and Sprint `5.7` full hot-path steelman with action-id generation, absolute side-to-move state, action-only tree storage, reusable wall masks, trusted internal buffers, and post-rewrite profile retuning. Sprint `8.14` remains historical current-artifact Haskell parity evidence for the pre-`5.7` target. | [Phase 5](phase-5-cpp-imperative-steelman.md), [Phase 8](phase-8-haskell-performance-parity-closure.md) |
| (iii) C++ functional-core | `cpp-functional` | `cpp-functional/` | C ABI via Haskell FFI | ✅ Done for compact value-state search, focused `(ii)` vs `(iii)` performance alignment, live parser/build/verify/FFI dispatch, Q3 participation, shared fail-closed C++ PGO/BOLT Plan/Apply wiring, compact ABI wording, Sprint `8.10` bounded played-game profile training, Sprint `8.11` bounded metric-suite profile training, and Sprint `6.9` hot-path shape alignment with backend (ii): absolute `SideToMove`, reusable `BlockMasks`, bidirectional path-existence BFS, action-only `UctNode`, and Sprint `5.8` C++ flag/BOLT scrub. Sprint `6.9` post-rebuild benches recorded backend (iii) at the cohort lead alongside backend (ii) on Q1a (`36303.9` playouts/s ST) and Q1b (`39180.3` search-iters/s ST). | [Phase 6](phase-6-cpp-functional-and-rust.md), [Phase 8](phase-8-haskell-performance-parity-closure.md) |
...
| (v) Haskell | `haskell` | `src/MCTS/Engine.hs`, `src/MCTS/Engine/Envelope.hs`, `src/MCTS/Driver.hs`, `src/MCTS/Search/Arena.hs`, `src/MCTS/Search/UCT.hs` | Native (in-process) | ✅ Done for Phase 3 correctness baseline, compact functional-core transition boundary, Sprint `8.12` parity refresh against corrected backend (ii), Sprint `8.13` style alignment, Sprint `8.14` report-card verdict/sample-stability gate, Sprint `8.15` measurement-vs-invariant reframe plus post-`5.7` `(ii)` rebaseline, Sprint `8.16` post-`5.8` rebaseline, and Sprint `8.17` `MutableByteArray# s` arena migration `measured but rejected` (single-buffer `STUArray s Int Word32` regressed focused Haskell rates by `Q1a -5.5%` / `Q1b -1.1%` ST against the Sprint `8.13` six-slab baseline; reverted under the Performance Measurement Doctrine; descent/rollout `INLINE` audit no-op). The hot search path uses packed numeric action IDs, compact pawn slots, direct packed-slot path starts, no-wall legal-action fast path, no-ply rollout apply, single-constructor action transitions, and `ST`-local arena internals behind the pure API. | [Phase 3](phase-3-haskell-engine.md), [Phase 8](phase-8-haskell-performance-parity-closure.md) |
