...
| In-process MCTS engine | `src/MCTS/Engine.hs`, `src/MCTS/Engine/Envelope.hs`, `src/MCTS/Driver.hs`, `src/MCTS/Search/Arena.hs`, `src/MCTS/Search/UCT.hs`, `src/MCTS/Verify.hs`; the Haskell engine uses strict `Word64` board slots/bitsets, real recursive UCT in the `ST` monad over a structure-of-arrays `STUArray` arena, deterministic `nonTerminalRank` tie-breaking, a sentinel `terminalOutcome` path for the rollout inner loop, and per-move search dispatch from the driver. The intended smoke command is `mcts verify rollouts --backend cpp-imperative,cpp-functional,rust,haskell` under the pinned toolchain | Replace in-process dispatch of foreign-named backends with real FFI-backed C++/Rust engines; Phase 8 owns performance-parity tuning |
| Foreign backend homes | `cpp-legacy/` contains the mechanically imported legacy core plus C ABI wrappers; `cpp-imperative/`, `cpp-functional/`, `rust/` contain smoke-buildable skeletons validated through `mcts build <backend>` inside the container; `src/MCTS/Driver/{CppLegacy,CppImperative,CppFunctional,Rust}.hs` can run bounded chosen-move smoke games through the real shared libraries via `MCTS.FFI.Common.withDynamicGame` | Full foreign game drivers with visit-vector instrumentation, C++23 engines, Rust engine, recompute ABI, PGO+BOLT+`mimalloc` pipelines |
| Test stanzas | `test/unit`, `test/integration`, `test/cross-backend`, `test/legacy-parity`, `test/haskell-style`; each stanza has its own `tasty` runner; validation gate is `cabal test all` under the pinned toolchain | Strengthen to external golden fixtures, real FFI cohort, renderer goldens, and full property coverage |
...
|---------|------------|---------------------|---------|--------|--------------|
| (i) C++ legacy port | `cpp-legacy` | `cpp-legacy/`, `src/MCTS/Driver/CppLegacy.hs` | C ABI via Haskell FFI | 🔄 Active (legacy core imported; bounded FFI smoke driver landed; full transcript driver/fixtures open) | [Phase 4](phase-4-cpp-legacy-port-and-ffi-bridge.md) |
| (ii) C++ imperative steelman | `cpp-imperative` | `cpp-imperative/`, `src/MCTS/Driver/CppImperative.hs` | C ABI via Haskell FFI | 🔄 Active (smoke skeleton with doctrine C++23 flags + `mimalloc` link and bounded FFI smoke driver; steelman open) | [Phase 5](phase-5-cpp-imperative-steelman.md) |
| (iii) C++ functional-style | `cpp-functional` | `cpp-functional/`, `src/MCTS/Driver/CppFunctional.hs` | C ABI via Haskell FFI | 🔄 Active (`mcts build cpp-functional` smoke path with matched C++23 flags + `mimalloc` link and bounded FFI smoke driver validated; real engine open) | [Phase 6](phase-6-cpp-functional-and-rust.md) |
| (iv) Rust | `rust` | `rust/`, `src/MCTS/Driver/Rust.hs` | C ABI via Haskell FFI, `cdylib` | 🔄 Active (`mcts build rust` smoke path, planned Rust module topology, `mimalloc` global allocator, and bounded FFI smoke driver validated; real engine open) | [Phase 6](phase-6-cpp-functional-and-rust.md) |
