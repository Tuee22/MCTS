...
surfaces in later phases remain closed on their owned code. Phase `6` Sprint `6.8`
has closed Rust hot-path alignment. Phase `7` Sprint `7.11` has closed Q7 semantic
parity, the `mcts-semantic-parity` stanza, and normalized divergence-score
reporting.
...
| (ii) C++ imperative steelman | `cpp-imperative` | `cpp-imperative/` | C ABI via Haskell FFI | ✅ Done for live parser/build/verify/FFI dispatch, Q3 participation, canonical fail-closed C++ PGO/BOLT Plan/Apply wiring, governed compact C ABI contract, Sprint `5.6` compact-board hot path, Sprint `8.10` bounded played-game profile training, Sprint `8.11` bounded metric-suite profile training, and Sprint `8.14` accepted Haskell parity target evidence. | [Phase 5](phase-5-cpp-imperative-steelman.md), [Phase 8](phase-8-haskell-performance-parity-closure.md) |
...
| (v) Haskell | `haskell` | `src/MCTS/Engine.hs`, `src/MCTS/Engine/Envelope.hs`, `src/MCTS/Driver.hs`, `src/MCTS/Search/Arena.hs`, `src/MCTS/Search/UCT.hs` | Native (in-process) | ✅ Done for the Phase 3 correctness baseline, compact functional-core transition boundary, Sprint `8.12` parity refresh against corrected backend (ii), Sprint `8.13` style alignment, and Sprint `8.14` fail-closed report-card verdict/sample-stability gate. The hot search path uses packed numeric action IDs and `ST`-local arena internals behind the pure API. | [Phase 3](phase-3-haskell-engine.md), [Phase 8](phase-8-haskell-performance-parity-closure.md) |
