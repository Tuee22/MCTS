...
| GCC | Latest stable on `ubuntu:24.04` | C++17 compiler for backend (i) cpp-legacy and C++23 compiler for backend (iii) cpp-functional. Sprint `5.9` pivoted backend (ii) off of GCC; (i) and (iii) clang pivots are tracked under Pending Removal in [legacy-tracking-for-deletion.md](legacy-tracking-for-deletion.md). | ✅ Done for the (i)/(iii) container baseline; (ii) is no longer a consumer | Sprint 4.1, Sprint 5.1 |
| `clang++-19` | `clang-19` package on `ubuntu:24.04` + `libclang-rt-19-dev` for PGO runtime | C++23 compiler for backend (ii) cpp-imperative. `llvm-profdata-19` merges `.profraw` files into `.profdata` for `-fprofile-use`. | ✅ Done for backend (ii) (Sprint `5.9` Makefile, `cppPgoBoltPlan` LLVM-profile branch, prerequisite probes for `cxx-clang19` and `llvm-profdata-19`) | Sprint 5.9 |
