...
| Transcript/cache baseline | `src/MCTS/Transcript.hs`, `src/MCTS/Crypto/SHA256.hs`, `src/MCTS/Transcript/EquitySidecar.hs`, plus wrapper modules; `.mcts-cache/` ignored; `inspect list` / `inspect show --envelope` / `inspect show --with-equity` / `inspect cache list` / `inspect cache prune` work; transcript and sidecar writes use same-directory temp files, fsync, rename, and parent-directory fsync; `MCTS.Engine.ForeignRecompute.foreignRecomputeEqStream` can score foreign recompute streams through `mcts inspect divergence` when cdylibs are present | Sidecar stale detection and verifier comparison against live FFI envelopes, replay-TUI cache-miss recompute writes, and report-card divergence matrix rendering |
...
| Foreign backend homes | `cpp-legacy/` contains the mechanically imported legacy core plus C ABI wrappers; `cpp-imperative/engine/` hosts the arena-MCTS imperative-steelman engine with the cohort 12-wall child cap; `cpp-functional/engine/` ships the functional-style API and `DescentStep`/`std::visit` descent data-flow over the same arena layout with the same wall cap; `rust/src/` ships a real arena MCTS plus xoshiro256++ plus the full Corridors gameplay port (8x8 bitfield walls, iterative BFS escapability, post-move 180-degree flip via `u64::reverse_bits`) and the visit-vector/recompute/read-visits C ABI; `src/MCTS/Driver/{CppLegacy,CppImperative,CppFunctional,Rust}.hs` dispatch through the real shared libraries via `MCTS.FFI.Common.withDynamicSearchGame` / `withDynamicRecomputeGame` | The `cpp-functional` / `rust` PGO+BOLT pipeline install closures and the Phase 7 post-move orientation gap |
| Test stanzas | `test/unit`, `test/integration`, `test/cross-backend`, `test/legacy-parity`, `test/haskell-style`; each stanza has its own `tasty` runner; host validation gate is `docker compose run --rm mcts mcts test all`; the Q6 legacy fixture set is committed under `test/golden/legacy/transcripts/arm64/` as a `1000`-simulation transitional snapshot; the Sprint 8.2 criterion micro-benchmark suite ships under `bench/criterion-suites.hs` + the `mcts-criterion` benchmark stanza in `mcts.cabal` | Final fine-grained tasty organization with full FFI-backed cohort, renderer goldens, full property coverage, and the `10000`-simulation fixture refresh through a `mcts` entrypoint |
...
|---------|------------|---------------------|---------|--------|--------------|
| (i) C++ legacy port | `cpp-legacy` | `cpp-legacy/`, `src/MCTS/Driver/CppLegacy.hs` | C ABI via Haskell FFI | ✅ Done for the Phase 4 closed surfaces (legacy core imported; full visit-vector FFI driver via `mcts_legacy_search_move`; Q6 fixtures committed under `test/golden/legacy/transcripts/`; envelope post-link patch + runtime CPU/FP probes + foreign-engine recompute landed; the `10000`-simulation fixture refresh is a ledger residue) | [Phase 4](phase-4-cpp-legacy-port-and-ffi-bridge.md) |
| (ii) C++ imperative steelman | `cpp-imperative` | `cpp-imperative/`, `src/MCTS/Driver/CppImperative.hs` | C ABI via Haskell FFI | ✅ Done for the Phase 5 closed surfaces (arena-MCTS engine with `Word16` ply counter, `thread_local` move buffer, `__builtin_prefetch`, `alignas(64)`; full visit-vector/recompute FFI driver; 11-step typed `Subprocess` PGO+BOLT pipeline with `llvm-bolt -instrument` self-recording; `-fno-exceptions` and the scratch-board residue are closed) | [Phase 5](phase-5-cpp-imperative-steelman.md) |
| (iii) C++ functional-style | `cpp-functional` | `cpp-functional/`, `src/MCTS/Driver/CppFunctional.hs` | C ABI via Haskell FFI | 🔄 Active (Sprints 6.1/6.2/6.5 closed at the worktree surface: arena-MCTS engine with `std::optional`/`std::variant` API, `DescentStep`/`std::visit` descent data-flow, visit-vector/recompute dispatch, live envelope probes, and typed `cppFunctionalPgoBoltPlan`; final PGO+BOLT install/verdict remains Phase 6/8 work) | [Phase 6](phase-6-cpp-functional-and-rust.md) |
| (iv) Rust | `rust` | `rust/`, `src/MCTS/Driver/Rust.hs` | C ABI via Haskell FFI, `cdylib` | 🔄 Active (Sprint 6.3 closed: real arena MCTS with UCT-1, xoshiro256++ native RNG, full `mcts_rust_search_move` / `mcts_rust_recompute_move` C ABI, cached `mcts_rust_read_visits`, AND the real Corridors gameplay implementation in `rust/src/board.rs` — 8x8 bitfield walls, iterative BFS escapability, post-move 180-degree flip via `u64::reverse_bits`; `mimalloc::MiMalloc` global allocator; `rustPgoBoltPlan` Plan/Apply harness via typed `Subprocess`. `MCTS.Driver.Dispatch.runBatchDispatch` routes `--backend rust` through the real FFI engine. End-to-end PGO+BOLT install closure remains active) | [Phase 6](phase-6-cpp-functional-and-rust.md) |
...
| Rollouts verify | `mcts verify rollouts` | Round-robin visit-count equality across the `(ii)..(v)` cohort under `--rng cpp`; baseline layered envelope checks with `--allow-stale` | 🔄 Active (real FFI dispatch for (ii)/(iii)/(iv); Sprint 7.2 selection-policy axis closed 2026-05-17 — Haskell `nonTerminalRank` heuristic dropped; C++ wall-child cap aligned on 2026-05-18; residual `VerifyMismatch` traces to post-move orientation conventions tracked in `legacy-tracking-for-deletion.md` Pending Removal) | Sprint 7.2, Sprint 7.5 |
| Self-play verify | `mcts verify selfplay` | Round-robin self-play visit-count equality across the `(ii)..(v)` cohort; baseline layered envelope checks with `--allow-stale` | 🔄 Active (real FFI dispatch for (ii)/(iii)/(iv); Sprint 7.2 selection-policy axis closed 2026-05-17 — Haskell `nonTerminalRank` heuristic dropped; C++ wall-child cap aligned on 2026-05-18; residual `VerifyMismatch` traces to post-move orientation conventions tracked in `legacy-tracking-for-deletion.md` Pending Removal) | Sprint 7.2, Sprint 7.5 |
| Legacy parity verify | `mcts verify legacy-parity {rollouts\|selfplay}` | 5-backend round-robin under `max_plies = 10000`, pinned fixture seed, `--rng cpp`; workload dispatched by `LegacyParityWorkload` | 🔄 Active (real FFI dispatch for (i)/(ii)/(iii)/(iv); the `10000`-simulation fixture refresh is a ledger residue) | Sprint 7.2 |
| Interactive game | `mcts play` | TUI human-vs-AI or AI-vs-AI spectate with live board rendering | 🔄 Active (non-interactive fallback plus interactive `brick` `App` event loop in `MCTS.CLI.Tui.Play` with legacy-notation move input (`MCTS.Notation.parseMove`), `:hint`/`:undo`/`:quit`, a `:save` placeholder, AI advance through in-process `uctSearch`, and a pure `applyUserInput` dispatcher covered by `mcts-unit::exerciseTuiPlayInput`; the shared board widget renders pawn cells plus horizontal/vertical wall segments; remaining work is transcript-save side effect, foreign-backend AI dispatch, and layout validation) | Sprint 7.4 |
...
| Per-backend envelope capture (build-id, compiler ID/version, fp_flags, libm_id, cpu_features, fp_env) | `<backend>/c-abi/envelope.{h,cc}` (Phase 4/5/6 for cpp/rust); `src/MCTS/Engine/Envelope.hs` (Phase 3 for haskell) | ✅ Done at the per-backend capture surface (Haskell logical envelope module; C++ runtime CPU/FP/libm probes + post-link build-id patch; Rust runtime feature/libm probes + build-id section patch). Verifier comparison against those live values is Sprint 7.5 remaining work. | Sprint 3.6, 4.7, 5.5, 6.5 |
...
| Layered envelope verify (`CohortLevel` + `BackendSlot` rule with `--allow-stale`) | `src/MCTS/Verify/Envelope.hs` | 🔄 Active (cohort-invariant logical v1 fields checked; backend-slot stale handling still compares against the logical expected envelope rather than live `mcts_<backend>_get_envelope()` values) | Sprint 7.5 |
...
| `forbiddenPathRegistry` (`.github/workflows/`, `.husky/`, `.githooks/`, `.pre-commit-config.yaml`, `pre-commit-*.yaml`, root `Makefile`/`justfile`/`Taskfile.yml`, host `.build/`, `bootstrap/`, repository `.sh` wrappers) | Lint, Format, and Code-Quality Stack → Forbidden Surfaces | 🔄 Active (typed `forbiddenPathRegistry :: [ForbiddenPath]` value in `MCTS.CLI.Lint`; the Compose-only doctrine update added `bootstrap/` and recursive `*.sh` refusal plus a unit expectation, but validation was stopped during Docker image build before `mcts-unit` ran) | Sprint 1.4 |
...
| `mcts-cross-backend` | Round-robin verify | Current `test/cross-backend/Main.hs` uses a `tasty` runner for the `verify` cohort under `--rng cpp` covering backends `(ii)`, `(iii)`, `(iv)`, `(v)` through `runBatchDispatch`; backend `(i)` is rejected by parser/runtime cohort checks while the final GADT shape remains open | 🔄 Active (logical fallback plus real FFI when cdylibs are present; residual post-move orientation equality open) | Sprint 7.2 |
| `mcts-legacy-parity` | Round-robin verify, legacy envelope | Current `test/legacy-parity/Main.hs` uses a `tasty` runner for `verify legacy-parity` across all five backends through `runBatchDispatch`; target pins `max_plies = 10000`, fixture seed `S_LP = 42`, and external legacy fixtures | 🔄 Active (logical fallback plus real FFI when cdylibs are present; external legacy fixture byte-compare open) | Sprint 7.2 |
...
| Docker development environment | `docker/Dockerfile`, root `compose.yaml` | `ubuntu:24.04` base with pinned GCC, LLVM, GHC, Cabal, Rust, a separate formatter-tools GHC for Fourmolu/HLint, copied project sources, and an installed `mcts`; all supported host work uses `docker compose run --rm mcts mcts <command>`. Repository `.sh` wrappers and `bootstrap/` helpers are forbidden workflow surfaces. Current validation is pending; the latest Compose run was stopped during Docker image build before the requested `mcts test mcts-unit` command ran |
