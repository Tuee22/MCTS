...
compact functional-core value-state style shared with backend (iv) and backend (v).
...
Phase `0` Sprint `0.3` is active on doctrine topology because the current worktree
does not contain `HASKELL_CLI_TOOL.md` while the plan and governed docs still cite it
as authoritative; implementation surfaces in later phases remain closed on their
owned code.
...
| Benchmark metric taxonomy | `documents/engineering/benchmark_metrics.md` defines terminal playout throughput (`playouts/s`), search-iteration throughput (`search-iters/s`), and played-game throughput (`games/s`). `mcts bench terminal-playouts` and `mcts bench search-iters` implement the primitive units across the five backend slots; `bench rollouts` remains a legacy command name for played-game throughput with one search iteration per real move; played-game benchmark renderers expose only `games/s`. The report card emits explicit Q1a/Q1b/Q2/Q5 unit fields, an aligned raw-performance table for every backend slot, a question-summary table that states each Q1a/Q1b/Q2/Q3/Q4/Q5/Q6 question, a divergence-matrix table, and a final observed-metric answer table for Q1a-Q6. Sprint `8.14` closed the current fail-closed parity rerun against corrected backend (ii), and the accepted report-card verdict is `Within tolerance`. | ✅ Done |
...
| Test runner | `mcts test all` / `mcts test <stanza>` | Plan/Apply over lint/docs prerequisites, Dockerfile-prebuilt Cabal test executables, verify cohorts, and the measured report-card workload. The report card times primitive Q1a/Q1b rows through direct benchmark runners, played-game Q2 rows through the no-write batch runner, records raw Q1a/Q1b/Q2 rates for every backend slot, and derives Q5 search-iteration and played-game scaling separately from the Haskell and backend (ii) rows, with typed prerequisite checks for Dockerfile-installed test/benchmark executables and Dockerfile-built backend artefacts before runtime apply; exact ordering and table layout are owned by `unit_testing_policy.md` | ✅ Done | Sprint 7.2, Sprint 7.3, Sprint 7.8, Sprint 7.10, Sprint 8.8, Phase 8 restoration |
...
| Doctrine topology | Repository worktree | `HASKELL_CLI_TOOL.md` (root; absent in the current checkout) | Sprint `0.3` owns restoring/updating this root doctrine file or retargeting every doctrine citation to a new canonical source |
...
| Doctrine topology | `HASKELL_CLI_TOOL.md` (absent in the current checkout) | Intended authoritative CLI doctrine at repo root; Sprint `0.3` owns restoring it or retargeting the doctrine references |
