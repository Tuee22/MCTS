...
surfaces in later phases remain closed on their owned code except active Phase `8`
Sprint `8.15`. Phase `6` Sprint `6.8` has closed Rust hot-path alignment. Phase
`7` Sprint `7.11` has closed Q7 semantic parity, the `mcts-semantic-parity`
stanza, and normalized divergence-score reporting.
...
report-card verdict is historical evidence for the Sprint `5.6` backend `(ii)`
artefact, and Phase `8` Sprint `8.15` is active on the Haskell shortfall exposed
by the fresh post-`5.7` parity rebaseline after accepting focused Haskell
hot-path work through direct packed-slot path starts, the no-wall legal-action
fast path, and single-constructor action transitions. The latest accepted aggregate
report-card rerun still fails closed with
...
| (v) Haskell | `haskell` | `src/MCTS/Engine.hs`, `src/MCTS/Engine/Envelope.hs`, `src/MCTS/Driver.hs`, `src/MCTS/Search/Arena.hs`, `src/MCTS/Search/UCT.hs` | Native (in-process) | 🔄 Active for Sprint `8.15` Haskell-vs-`(ii)` parity. Done surfaces remain the Phase 3 correctness baseline, compact functional-core transition boundary, Sprint `8.12` parity refresh against corrected backend (ii), Sprint `8.13` style alignment, and Sprint `8.14` fail-closed report-card verdict/sample-stability gate. The hot search path uses packed numeric action IDs, compact pawn slots, direct packed-slot path starts, no-wall legal-action fast path, no-ply rollout apply, single-constructor action transitions, and `ST`-local arena internals behind the pure API. | [Phase 3](phase-3-haskell-engine.md), [Phase 8](phase-8-haskell-performance-parity-closure.md) |
