...
across `(i)..(v)`. Phase `8` closes the restoration and the rule that normal
validation does not depend on checked-in generated validation data; the
optimized-C++ performance parity refresh closed on 2026-05-21 after Sprint `5.3`
routed the C++ PGO/BOLT Plan/Apply path through `mcts build`.
...
| Foreign backend homes | `cpp-legacy/` contains the mechanically imported legacy core plus C ABI wrappers and the optional local `legacy-to-wire` evidence generator reached through `mcts build legacy-fixtures`; `cpp-imperative/` contains the imperative steelman; `cpp-functional/` contains the functional-style C++ steelman; `rust/src/` ships a real arena MCTS, a dormant xoshiro256++ helper module, the splitmix-compatible live search schedule, the full Corridors gameplay port (8x8 bitfield walls, iterative BFS escapability, post-move 180-degree flip via `u64::reverse_bits`), and the visit-vector/recompute/read-visits C ABI. Haskell dispatch loads all four foreign backend families through `src/MCTS/FFI/Cpp*`, `src/MCTS/FFI/Rust.hs`, and `src/MCTS/Driver/Dispatch.hs` when canonical shared libraries are present. | ✅ Done |
...
| (ii) C++ imperative steelman | `cpp-imperative` | `cpp-imperative/` | C ABI via Haskell FFI | ✅ Done (live parser/build/verify/FFI dispatch, Q3 participation, and canonical C++ PGO/BOLT Plan/Apply wiring) | [Phase 5](phase-5-cpp-imperative-steelman.md), [Phase 8](phase-8-haskell-performance-parity-closure.md) |
| (iii) C++ functional-style | `cpp-functional` | `cpp-functional/` | C ABI via Haskell FFI | ✅ Done (live parser/build/verify/FFI dispatch, Q3 participation, and shared C++ PGO/BOLT Plan/Apply wiring) | [Phase 6](phase-6-cpp-functional-and-rust.md), [Phase 8](phase-8-haskell-performance-parity-closure.md) |
...
| Divergence matrix | `mcts inspect divergence <hash-prefix>` | Emit divergence-rate metrics (`visit-Δ`, `move-Δ`, `equity-L2`) for a single transcript and cached backend columns; report-card rows consume decoded verify transcripts consistent with the corrected canonical comparator | ✅ Done | Sprint 7.5, Phase 8 restoration |
| Test runner | `mcts test all` / `mcts test <stanza>` | Plan/Apply over backend builds, Cabal test stanzas, verify cohorts, and the no-write measured report-card workload | ✅ Done | Sprint 7.2, Sprint 7.3, Sprint 8.8, Phase 8 restoration |
...
| Per-backend build harness | `mcts build cpp-legacy`, `mcts build cpp-imperative`, `mcts build cpp-functional`, `mcts build rust`, plus `mcts build legacy-fixtures` | Plan/Apply per-backend build; C++ and Rust steelman leaves run their PGO+BOLT+`mimalloc` paths, and the optional local legacy evidence generator remains external/ignored | ✅ Done | Sprint 4.5, Sprint 5.3, Sprint 6.2, Sprint 6.4, Sprint 8.8, Phase 8 restoration |
...
| Foreign-engine recompute (`mcts_<backend>_recompute_move` FFI; in-process `Recompute.hs` for haskell) | per-backend C ABI + `src/MCTS/Engine/Recompute.hs` + `src/MCTS/Engine/ForeignRecompute.hs` | ✅ Done (in-process `Recompute.hs` covers Haskell; C++ and Rust expose live recompute ABIs with parent-perspective `chosen_equity` where their shared libraries are present) | Sprint 3.6, 4.7, 5.5, 6.5, Phase 8 restoration |
...
| BOLT | LLVM `19` package in `docker/Dockerfile` | Post-link binary reordering after PGO for backends (ii), (iii), (iv) | ✅ Done for Docker pinning, prerequisite probing, Rust BOLT training/install, and the supported C++ build wiring. C++ Makefile PGO/BOLT targets use the documented PGO fallback when no usable `.fdata` is produced. | Sprint 5.3, Sprint 6.4 |
...
| PGO profile directories | Two-stage PGO build harness | `cpp-imperative/pgo-profile/`, `cpp-functional/pgo-profile/`, `rust/pgo-profile/` | Rust and C++ directories are created and consumed by their supported `mcts build <backend>` PGO/BOLT paths |
