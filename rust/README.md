...
train/merge/use with `-C target-cpu=native -C link-arg=-fuse-ld=lld`, BOLT
training/install on amd64, canonical `rust/target/release/libmcts_rust.so`
publication, and post-link `engine_build_id` patching through the Compose
entrypoint.
